...
// ----------- Parse format string -----------------
inline bool skip_to_format(rikitiki::stringstream& b, const rikitiki::string::value_type *& format){
  while( *format != '{' ){
...
inline bool skip_to_unformat(const rikitiki::string::value_type *& format){
  while( *format != '}' ){
...
template <typename T>
inline static bool extract(rikitiki::stringstream& b, T& t){
  b >> t;  
  return !b.fail();
}

template <>
inline bool extract(rikitiki::stringstream& b, rikitiki::string& t){
  while(b.good()){
    auto v = b.get();
    bool accept = 
      (v >= L'0' && v <= L';') ||
...
static inline int modern_sscanf(rikitiki::stringstream& b, const rikitiki::string::value_type *& format){
  while( *format != '\0' && !b.eof() ){
    if((rikitiki::string::value_type )b.peek() != *format) return false;
    b.get(); if(!b.good()) return 0;
    format++;
  }       
  return *format == '\0' && (rikitiki::string::value_type )EOF == (rikitiki::string::value_type )b.peek() ? 1 : 0;
} 

template <typename H, typename... T>
static int modern_sscanf(rikitiki::stringstream& b, const rikitiki::string::value_type *& format, H& h, T&... t){
...
template <typename... T>
static int modern_sscanf(const rikitiki::string::value_type * b, const rikitiki::string::value_type * format, T&... t){
  rikitiki::stringstream ss(b);
  return modern_sscanf(ss, format, t...);
}

//------------------ Route_ -----------------------
template <typename P, typename... T> 
Route_<P,T...>::Route_(P* p, const rikitiki::string& _route, F _f, Request::Method method) : parent(p), f(_f), Route(_route, method) { }
...
bool Route_<P>::Handle(ConnContextRef ctx){
     Handler::Handle(ctx);
  bool shouldAttempt =
          strcmp(route.c_str(), ctx->Request.URI()) == 0 &&
    (method == RequestMethod::ANY ||
