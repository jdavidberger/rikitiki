...
namespace rikitiki {
	namespace mongoose {
		...
		MongooseWebsocketContext::MongooseWebsocketContext( Server* s, const mg_connection* conn) :
			MongooseRequestContext(mg_get_request_info(const_cast<mg_connection*>(conn))),
			requestConn(conn), conn(0), WebsocketContext(server) {

		}
		MongooseWebsocketContext::MongooseWebsocketContext( Server* s, mg_connection* conn) : requestConn(conn), conn(conn), MongooseRequestContext(mg_get_request_info(conn)), WebsocketContext(server)  {
