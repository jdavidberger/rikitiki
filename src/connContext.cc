...
namespace rikitiki {
     ...
     HeaderCollection::value_type& RequestContext::AddRequestHeader(const char* _name, const char* value){
     ...
     RequestContext::RequestContext() : mappedQs(false), mappedHeaders(false), mappedCookies(false) {}

     ConnContext::ConnContext(Server* _server) : _method(ANY),
          handled(false),
          server(NULL),
          mappedPost(false),
          mappedPayload(false),
